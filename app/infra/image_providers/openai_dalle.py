...
class OpenAIImageProvider:
    ...
    async def generate_image(
        ...
        **kwargs: Any
        ...
        job_id = str(uuid4())
        logger.info(f"🎨 Starting OpenAI image generation [{job_id[:8]}]: '{prompt[:50]}...'")
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if response.status_code != 200:
                    ...
                    logger.error(f"❌ OpenAI API error [{job_id[:8]}]: {response.status_code} - {error_detail}")
                ...
                if image_response.status_code != 200:
                    logger.error(f"❌ Failed to download image [{job_id[:8]}]: {image_response.status_code}")
                    raise Exception(f"Failed to download generated image: {image_response.status_code}")
                
                image_bytes = image_response.content
                content_type = image_response.headers.get("content-type", "image/png")
                
                logger.info(f"✨ OpenAI image generated [{job_id[:8]}]: {len(image_bytes)} bytes")
            ...
            logger.error(f"⏰ OpenAI API timeout for [{job_id[:8]}] after {settings.request_timeout_s}s")
