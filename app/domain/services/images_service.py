...
class ImagesService:
    ...
    async def generate_image(
        ...
        logger.info(
            f"🎨 Starting image generation for: '{request.prompt_brief[:50]}...'"
        )
        ...
        try:
            ...
            if request.aspect_ratio not in supported_ratios:
                raise ValueError(
                    f"Unsupported aspect ratio: {request.aspect_ratio}. Supported: {supported_ratios}"
                )

            # Validate seed range if provided
            if request.seed is not None and (
                request.seed < 0 or request.seed > 2147483647
            ):
    ...
    async def health_check(self) -> dict:
        ...
        try:
            ...
            health_status = {
                "status": (
                    "healthy" if (openai_healthy and storage_healthy) else "degraded"
                ),
                "services": {
                    "openai_dalle": {
    ...
    def get_artifact_info(self, job_id: str) -> dict | None:
    ...
    async def regenerate_image(
        self, job_id: str, modifications: dict | None = None
    ) -> ImageGenerateResponse | None:
