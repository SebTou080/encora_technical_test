...
def sample_excel_content():
    ...
    df = pd.DataFrame(
        [
            {
                "comment": "Me encantan estos chips",
                "username": "user1",
                "sku": "KALE-90G",
                "channel": "web",
            },
            {
                "comment": "Sabor regular",
                "username": "user2",
                "sku": "QUINOA-80G",
                "channel": "app",
            },
...
class TestFeedbackChain:
    ...
    def test_validate_analysis(self):
        ...
        assert (
            validated.issue_priority in ["alta", "media", "baja"]
            or validated.issue_priority is None
        )
...
class TestFeedbackService:
    ...
    def test_create_upload_file_csv(self, sample_csv_content):
        ...
        return UploadFile(
            filename="test_feedback.csv", file=io.BytesIO(sample_csv_content)
        )

    def test_create_upload_file_excel(self, sample_excel_content):
        """Helper to create UploadFile for Excel."""
        return UploadFile(
            filename="test_feedback.xlsx", file=io.BytesIO(sample_excel_content)
        )
    ...
    async def test_parse_csv_file(self, sample_csv_content):
        ...
        upload_file = UploadFile(
            filename="test.csv", file=io.BytesIO(sample_csv_content)
        )
    ...
    async def test_parse_excel_file(self, sample_excel_content):
        ...
        upload_file = UploadFile(
            filename="test.xlsx", file=io.BytesIO(sample_excel_content)
        )
    ...
    async def test_parse_invalid_file_format(self):
        ...
        upload_file = UploadFile(
            filename="test.txt", file=io.BytesIO(b"invalid content")
        )
    ...
    async def test_parse_missing_required_columns(self):
        ...
        upload_file = UploadFile(filename="invalid.csv", file=io.BytesIO(invalid_csv))
    ...
    async def test_column_name_variations(self):
        ...
        upload_file = UploadFile(filename="spanish.csv", file=io.BytesIO(spanish_csv))
    ...
    async def test_analyze_file_complete_flow(
        self,
        mock_aggregate,
        mock_analyze_batch,
        sample_csv_content,
        mock_comment_analysis,
    ):
    ...
    async def test_save_analysis_results(self, mock_storage):
        ...
        assert (
            mock_storage.save_metadata.call_count == 2
        )  # Two saves: results + export data
    ...
    def test_get_analysis_info(self):
        ...
        with patch("app.infra.storage.storage") as mock_storage:
            ...
            mock_storage.list_job_artifacts.return_value = [
