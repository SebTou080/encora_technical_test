...
from ..infra.image_providers.openai_dalle import (
    OpenAIImageProvider,
    OpenAIImageResponse,
)
...
def mock_storage_service():
    ...
    mock_storage.save_metadata.return_value = (
        "./data/artifacts/test-job-123/metadata.json"
    )
    ...
    mock_storage.get_artifact_path.return_value = (
        "/app/data/artifacts/test-job-123/image.png"
    )
    return mock_storage
...
class TestOpenAIImageProvider:
    ...
    async def test_generate_image_success(
        self, mock_get, mock_post, sample_image_request
    ):
        ...
        result = await provider.generate_image(
            prompt=sample_image_request.prompt_brief, size="1024x1024"
        )
...
class TestStorageService:
    ...
    def test_save_and_load_image(self, tmp_path, monkeypatch):
        ...
        storage.save_image(job_id, image_data, "test.png")

        # Verify file exists and content is correct
        full_path = (
            tmp_path / "test_storage" / "artifacts" / "test-job-123" / "test.png"
        )
...
class TestImagesService:
    ...
    async def test_generate_image_success(
        self, mock_chain_generate, sample_image_request
    ):
    ...
    async def test_health_check_all_healthy(
        self, mock_storage_stats, mock_openai_health
    ):
        ...
        with patch.object(service, "generate_image") as mock_generate:
            ...
            with patch("app.infra.storage.storage.load_metadata") as mock_load_meta:
                mock_load_meta.return_value = {
                    "request": sample_image_request.model_dump()
                }

                result = await service.regenerate_image(
                    "old-job-123", {"prompt_brief": "new improved prompt"}
                )
